...
## Loop 4 — Framework gaps surfaced by dogfooding (for `framework/GAPS.md`)

Processing this instance's own content through the framework (SP11 rehearsal) is the first real adoption, so the gaps it surfaced are Loop-4 co-evolution feedback. **Recorded here rather than written into `framework/GAPS.md` on purpose:** that register is Session 1's live artifact, and the handoff keeps Sessions 1 & 2 file-disjoint to avoid clobbering concurrent work. These should be folded into `framework/GAPS.md` once Session 1 is done.

1. **CLI `validate` can't validate a collection file.** `node …/cli.mjs validate <schema> <file>` reads the whole YAML as one object, so it reports "invalid" on a `{ entries: [...] }` / `{ resources: [...] }` list file (the wrapper has no `title`/`type`). Tasks 3–6 had to validate **per-object** via the programmatic `validateObject` in a loop. → *Recommendation:* add a list/batch validate mode to the CLI (e.g. `validate <schema> <file> --list <key>`), or document the collection convention. This is purely a CLI ergonomics gap — the API is correct.
2. **The framework package can't be imported into an Astro/Vite build.** `import { listSchemas, validateKernel } from '…/toolkit-framework/src/index.mjs'` fails when bundled: the package resolves `SCHEMA_DIR` from `import.meta.url`, which Vite rewrites into `dist/.prerender/chunks/`, so the schema YAML no longer resolves (`schema not found: core-entities`; `listSchemas()` silently returns `[]`). Task 7 worked around it with a Node **prebuild manifest** (`scripts/gen-framework-manifest.mjs` → `src/data/framework-manifest.json`). → *Recommendation:* ship a build-safe consumption path — a precompiled/exported manifest, schemas inlined as JS, or a documented "snapshot via a Node prebuild" pattern (the one we used). Any framework consumer inside a bundler hits this.
3. **`track.outcome` is a scalar string, but real journeys carry an array of outcomes.** Heenal's journeys list 3–5 discrete outcomes each; Task 4 had to join them into one `"; "`-separated string to satisfy the schema, flattening the structure. → *Recommendation:* make `outcome` an `array` (or add `outcomes: array`) on the `track` schema.
4. **No first-class `public_use_boundary` field on content schemas.** Task 6 attaches a `public_use_boundary` sub-object to 34 high-risk entries; it validates only because `encyclopedia-entry extends frontmatter` allows extra fields — it isn't a declared, checkable field. → *Recommendation:* declare an optional `public_use_boundary` (or `boundaries: array`) on `encyclopedia-entry` / `resource`, referencing the existing `public-use-boundary` schema, so the linkage is first-class.
5. **"Held / review-queue" rows have no typed home.** Task 2's lift held 698 rows (social-signal-review, account-review, structural-integrity-review, safeguards, automated-curation) out of the Resource Graph because the framework has no review-queue lifecycle target — they live only in the CSV + this report. → *Recommendation:* a triage/review-queue state (or a lightweight `signal-review` object) so held rows have a typed destination instead of being counted-but-homeless.
6. **Standardize salvage/provenance.** Task 5 added ad-hoc `salvaged_from` provenance to 144 salvaged entries; a `provenance` schema already exists but content schemas don't reference it. → *Recommendation:* a standard optional provenance field on the content schemas (or document `salvaged_from`/`source_lineage` as the convention).

(Corroborating, already noted in the Task 2 section above: the `source-system` `type` enum is too narrow — 60 of 89 fell back to `database`. Widen the enum or add `organization`/`other`.)
