...
- **Set a review-by date** in each page's metadata. When it lapses, the page's trust state drops back to "needs review" automatically in your mind, and ideally in a query.
...
How you handle that disagreement *is* trust infrastructure. **GravityDAO**, which grew out of the Token Engineering Commons, argues that smart contracts alone can't build healthy communities — you also need ways to process conflict, and they lean on nonviolent communication: describe experiences and needs without assigning blame [GravityDAO]. Bring that posture into reviews. Comment on the claim, not the person. The goal is a more trustworthy page, and everyone is on the same side of that.
