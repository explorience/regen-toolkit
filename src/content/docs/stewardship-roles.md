...
The Regen Knowledge Commons Toolkit treats this as a real concern, not an afterthought. One of its cross-cutting principles is **living systems health**: paying attention to "energy, trust, contribution flow, maintenance capacity" across the whole commons, not just whether the docs are technically correct ([Regen Toolkit, Cross-cutting principles](https://regen-toolkit.superbenefit.dev)). A page can be accurate and still be dying, because nobody is updating it, nobody is linking to it, and nobody feels responsible for it.
