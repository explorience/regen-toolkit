...
- [2026-07-05] **`@org-os/kms` built + installed** — the org-os binding for the 0.2 framework (module `src/bind.mjs` + org-os profile; full 6-piece module — executable lifecycle, registry bridge, CLI, render, federation, e2e gate; 44 tests, subagent-driven TDD). **Hybrid architecture:** thin orchestration over the `@regen-commons/toolkit-framework` public API + declarative registry/lifecycle bindings; reimplements no framework logic. Develop-in-place in `regen-toolkit/packages/org-os-kms/`; canonical home `org-os/packages/`. Activated in regen-toolkit (`init`) + promoted to canonical org-os (`v0.5`) + ReFi DAO pilot initialized. Design/plan: [`docs/plans/2026-07-05-org-os-kms-design.md`](docs/plans/2026-07-05-org-os-kms-design.md) / [`-implementation.md`](docs/plans/2026-07-05-org-os-kms-implementation.md). Per-instance data processing deliberately **not** batched here — runbook: [`docs/plans/2026-07-05-org-os-kms-per-instance-adoption.md`](docs/plans/2026-07-05-org-os-kms-per-instance-adoption.md).
- [2026-07-05] **Ontology comparison → HYBRID (adopt-with-contribute-back)** — Heenal's build (**19** core types, 9 attribute-layers, Octo-anchored) vs the framework kernel (15 core + 31 extension, 10 layers, K1 3-axis state). Heenal's 119 live articles reprocess into framework objects **1:1**. Adopt the framework as the operational + interop backbone; keep Heenal's capital accounting (`stage`/`scale`/`context`) as a namespaced extension; **contribute the 8 Forms of Capital back to the kernel** — the one regenerative construct the framework lacks. Report: [`docs/reports/2026-07-05-ontology-comparison.md`](docs/reports/2026-07-05-ontology-comparison.md); crosswalk + diff: `data/crosswalks/`; frozen baseline: `data/ontology/snapshot-2026-07-05/`. Share-pack (group + Matty) drafted: [`docs/reports/2026-07-05-share-pack.md`](docs/reports/2026-07-05-share-pack.md).
- [2026-07-04] **Framework 0.2.0 "the machine" — built, merged, extracted, shipped.** The 0.1 skeleton (schemas + kernel) became a functional **agent-native ingestion machine**: point it at raw sources → agents draft candidate typed objects → the CLI accept gate validates → a review queue holds them → a named human promotes → objects store through a swappable adapter. Load-bearing architectural decisions: **(1) three seams** — agents never touch storage (work-order contract), ingestion ≠ storage (adapter interface: kb-folder/repo-data/geo-stub — Durgadas's constraint made structural), data ≠ site (derived index + JSON-LD); **(2) nothing auto-promotes** — everything born `raw`+`ai_assisted`, only a named human moves it (K1 maturity: raw/draft/candidate/source-linked/reviewed — *no "plausible"*); **(3) replication is first-class** — `cli init --new/--existing` + `federate add/check`, every KB born a federation citizen (self source-system card, opaque adapter refs); **(4) built here, extracted to a public repo** [`luizfernandosg/toolkit-framework`](https://github.com/luizfernandosg/toolkit-framework) — portability proven by `npx degit` running green outside the monorepo. 100/100 tests (every task two-stage-reviewed; criticals caught: path-traversal via candidate `file:` key, meta-schema validation bypass, peer-card self-card hijack, torn-state promote). First real ingestion: the 2026-07-02 planning call → 10 typed objects. Merged `a885d7f`, tag `toolkit-framework-v0.2.0`. Site pages (`/framework`, `/regen-toolkit-os`) updated to reflect the machine + pushed (Pages preview live). Full log: [`memory/2026-07-04.md`](memory/2026-07-04.md); spec+plan: [`docs/plans/framework-build/2026-07-04-machine-*.md`](docs/plans/framework-build/).
